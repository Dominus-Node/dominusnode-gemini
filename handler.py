"""Dominus Node Gemini / Vertex AI function calling handler (Python).
...
def _is_private_ip(hostname: str) -> bool:
    ...
    if ip_lower.startswith("::") and not ip_lower.startswith("::ffff:"):
        rest = ip_lower[2:]
        if rest and "." in rest:
            return _is_private_ip(rest)
        parts = rest.split(":")
        if len(parts) == 2 and parts[0] and parts[1]:
            try:
                hi = int(parts[0], 16)
                lo = int(parts[1], 16)
                reconstructed = f"{(hi >> 8) & 0xFF}.{hi & 0xFF}.{(lo >> 8) & 0xFF}.{lo & 0xFF}"
                return _is_private_ip(reconstructed)
            except ValueError:
                pass
...
def create_dominusnode_function_handler(
    ...
    def _validate_daily_limit(value: Any) -> Optional[str]:
        if isinstance(value, bool) or not isinstance(value, int):
            return "daily_limit_cents must be a positive integer"
        if value < 1 or value > 1_000_000:
            return "daily_limit_cents must be between 1 and 1,000,000"
        return None

    def _validate_domains(value: Any) -> Optional[str]:
    ...
    async def handle_create_agentic_wallet(args: Dict[str, Any]) -> str:
        ...
        daily_limit = args.get("daily_limit_cents")
        if daily_limit is not None:
            err = _validate_daily_limit(daily_limit)
            if err:
        ...
        if allowed is not None:
            err = _validate_domains(allowed)
            if err:
                return json.dumps({"error": err})
            body["allowedDomains"] = allowed
    ...
    async def handle_topup_crypto(args: Dict[str, Any]) -> str:
        ...
        currency = str(args.get("currency", "")).lower()
        ...
        if currency not in _VALID_CRYPTO_CURRENCIES:
            return json.dumps({
                "error": f"currency must be one of: {', '.join(sorted(_VALID_CRYPTO_CURRENCIES))}"
            })

        result = await api_request(
            "POST",
            "/api/wallet/topup/crypto",
            {"amountUsd": amount_usd, "currency": currency},
        )
        return json.dumps(result)
    ...
    async def handle_update_wallet_policy(args: Dict[str, Any]) -> str:
        ...
        if daily_limit is not None:
            ...
            if err:
                return json.dumps({"error": err})
            body["dailyLimitCents"] = daily_limit
        elif "daily_limit_cents" in args:
            # Explicitly passed as None (null) to clear
    ...
    async def handler(name: str, args: Dict[str, Any]) -> str:
        """Dispatch a Gemini function call to the appropriate Dominus Node API endpoint.
